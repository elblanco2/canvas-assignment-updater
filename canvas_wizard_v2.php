<?php
/**
 * Canvas Assignment Due Date Updater - Version 2.0
 * 
 * AI-powered Canvas assignment due date management with manual matching and session control
 * 
 * Features:
 * - Multi-step wizard interface
 * - AI-powered assignment matching (Gemini/Claude)
 * - Canvas API integration
 * - Manual assignment matching for missed items
 ...
 * @license MIT
 * @repository https://github.com/elblanco2/canvas-assignment-updater
 */

// Version information
define('APP_VERSION', '2.0.0');
define('APP_NAME', 'Canvas Assignment Updater');
define('GITHUB_REPO', 'https://github.com/elblanco2/canvas-assignment-updater');
...
function fetchCanvasAssignments($canvas_url, $api_token, $course_id) {
    ...
    if ($httpCode !== 200) {
        throw new Exception("Failed to fetch Canvas assignments: HTTP $httpCode - $response");
    }
...
function updateCanvasAssignments($canvas_url, $api_token, $course_id, $matches) {
    ...
    foreach ($matches as $match) {
        try {
            ...
            // Get assignments if not cached
            if (!isset($_SESSION['canvas_assignments'])) {
                $_SESSION['canvas_assignments'] = fetchCanvasAssignments($canvas_url, $api_token, $course_id);
            }
            
            $assignments = $_SESSION['canvas_assignments'];
            $assignment_id = null;
...
// Handle AJAX fetch Canvas assignments
if (isset($_POST['ajax']) && $_POST['ajax'] === 'fetch_canvas') {
    header('Content-Type: application/json');
    ...
    try {
        ...
        if (empty($canvas_url) || empty($api_token) || empty($course_id)) {
            echo json_encode(['error' => 'Missing Canvas credentials']);
            exit;
        }
        
        $assignments = fetchCanvasAssignments($canvas_url, $api_token, $course_id);
        $_SESSION['canvas_assignments'] = $assignments;
    ...
    try {
        ...
        switch ($action) {
            ...
            case 'logout':
                ...
                $_SESSION['current_step'] = '1';
                $current_step = '1';
                $success = "Session cleared! All API keys and data removed.";
                break;
            ...
            <div class="wizard-header">
                <div class="session-controls">
                    ...
                    <form method="POST" style="display: inline;">
                        ...
                        <button type="submit" class="btn btn-secondary" style="font-size: 12px; padding: 5px 15px;">🔄 Reset</button>
                    </form>
                </div>
                
                <div class="version-info">
                    v<?php echo APP_VERSION; ?> | <a href="<?php echo GITHUB_REPO; ?>" target="_blank" style="color: #3498db;">GitHub</a>
                </div>
                
                <h1>🎨 <?php echo APP_NAME; ?></h1>
                <p>AI-powered assignment due date management with manual matching</p>
            </div>
...
..."
                ...
                <?php if (!empty($_SESSION['matches'])): ?>
                        ...
                        <tbody>
                            <?php foreach ($_SESSION['matches'] as $match): ?>
                                <tr>
                                    ...
                                    <td><?php echo date('g:i A T', strtotime($match['matched_due_date'])); ?></td>
                    ...
                    <!-- Manual Matching Section for V2 -->
                    <div class="manual-match-section">
                        <h3>🔧 Manual Assignment Matching</h3>
                        <p>Fetch your Canvas assignments to manually match any that the AI missed.</p>
                        
                        <div class="form-group">
                            <label>Canvas API URL</label>
                            <input type="text" id="canvas_url_preview" 
                        ...
                        <div class="form-group">
                            ...
                            <input type="password" id="api_token_preview" 
                                   placeholder="Your Canvas API token">
                        </div>
                        
                        <div class="form-group">
                            <label>Course ID</label>
                            <input type="text" id="course_id_preview" 
                                   placeholder="453" 
                                   value="<?php echo htmlspecialchars($_POST['course_id'] ?? ''); ?>">
                        </div>
                        
                        <button type="button" class="btn btn-secondary" onclick="fetchCanvasAssignments()">
                            📥 Fetch Canvas Assignments
                        </button>
                        
                        <div id="canvas-assignments-list" class="canvas-assignment-list" style="display: none;">
    ...
    <div class="ai-sidebar">
        ...
        <div class="ai-chat">
            <div class="chat-messages" id="chatMessages">
                ...
                <?php elseif ($current_step == '3'): ?>
                    <div class="chat-message ai">
                        <strong>AI:</strong> Perfect! Your assignments are matched and ready for Canvas update. I can see <?php echo count($_SESSION['matches'] ?? []); ?> assignments will be updated for Fall 2025. Review the preview above and enter your Canvas credentials to apply the changes.
    ...
    <script>
        ...
        // Fetch Canvas assignments for manual matching
        function fetchCanvasAssignments() {
            const canvasUrl = document.getElementById('canvas_url_preview').value;
            const apiToken = document.getElementById('api_token_preview').value;
            const courseId = document.getElementById('course_id_preview').value;
            
            if (!canvasUrl || !apiToken || !courseId) {
                alert('Please fill in all Canvas credentials');
                return;
            }
            
            const assignmentsList = document.getElementById('canvas-assignments-list');
            assignmentsList.style.display = 'block';
            assignmentsList.innerHTML = '<p>Loading Canvas assignments...</p>';
            ...
            fetch('', {
                ...
                body: `ajax=fetch_canvas&canvas_url=${encodeURIComponent(canvasUrl)}&api_token=${encodeURIComponent(apiToken)}&course_id=${encodeURIComponent(courseId)}`
            })
            .then(response => response.json())
            .then(data => {
                if (data.error) {
                    assignmentsList.innerHTML = '<p style="color: red;">Error: ' + data.error + '</p>';
                } else {
                    let html = '<h4>Canvas Assignments Found:</h4>';
                    data.assignments.forEach(assignment => {
                        html += `<div class="assignment-item" onclick="selectCanvasAssignment('${assignment.name}', ${assignment.id})">
                            <strong>${assignment.name}</strong> (ID: ${assignment.id})
                            <br><small>Due: ${assignment.due_at || 'Not set'}</small>
                        </div>`;
                    });
            ...
            .catch(error => {
                assignmentsList.innerHTML = '<p style="color: red;">Error fetching assignments: ' + error.message + '</p>';
            });
        }
        
        function selectCanvasAssignment(name, id) {
            // This would be used for manual matching functionality
            console.log('Selected assignment:', name, id);
            alert(`Selected: ${name}\nYou can manually match this with your schedule.`);
        }
    </script>
</body>
</html>